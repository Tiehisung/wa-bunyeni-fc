...
const matchApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getMatches: builder.query<IQueryResponse<IMatch[]>, IQueryParams>({
      query: (params) => ({
        url: "/matches",
        params,
      }),
    ...
    getMatch: builder.query<IQueryResponse<IMatch>, string>({
      ...
      providesTags: (_result, _error, slugOrId) => [
        { type: "Matches", id: slugOrId },
      ],
    }),

    // GET upcoming matches
    getUpcomingMatches: builder.query<
      IQueryResponse<IMatch[]>,
      {
        limit?: number;
        team?: string;
        tournament?: string;
    ...
    // GET matches by team
    getMatchesByTeam: builder.query<
      IQueryResponse<IMatch[]>,
      {
        teamId: string;
        page?: number;
    ...
    updateLiveMatchEvents: builder.mutation<
      IQueryResponse<IMatch>,
      {
        matchId: string;
        event: {
    ...
    bulkUpdateLiveEvents: builder.mutation<
      IQueryResponse<IMatch>,
      {
        matchId: string;
        events: Array<{
    ...
    updateMatchMinute: builder.mutation<
      IQueryResponse<IMatch>,
      {
        matchId: string;
        minute: number;
    ...
    // BULK delete matches
    bulkDeleteMatches: builder.mutation<
      IQueryResponse<{ deletedCount: number }>,
      string[]
    >({
      query: (matchIds) => ({
